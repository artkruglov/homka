 ...
 * - Age counts from creation or the last reinforcement; the event date does not age a record.
...
describe("memoryRetention", () => {
  ...
  it("decays from learning or the last reinforcement, never from the event date", () => {
    ...
    // An old event learned today is new knowledge (upstream 960c4b9): it must not fade on arrival.
    const learnedToday = memoryRetention({ ...base, createdAt: now, lastReinforcedAt: null, occurredAt: daysAgo(100) }, now);
    expect(learnedToday).toBeCloseTo(1, 5);
    expect(isRetainedForAutomaticContext(learnedToday)).toBe(true);
