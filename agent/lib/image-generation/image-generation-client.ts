...
export interface ImageGenerationRequest {
  references?: Array<{ bytes: Buffer; mediaType: ImageMediaType }>;
...
function productionClient(): { assertConfigured(): void; generate(input: ImageGenerationRequest): Promise<GeneratedImage> } {
  // Explicit image credentials choose a separate provider without changing the conversational model.
  // Partial configuration fails closed instead of silently charging a different provider.
  if (process.env.OPENROUTER_IMAGE_API_KEY || process.env.OPENROUTER_IMAGE_MODEL) {
    return createOpenRouterImageClient({apiKey: process.env.OPENROUTER_IMAGE_API_KEY ?? '',
...
export const imageGenerationClient = {
  assertConfigured(input?: ImageGenerationRequest): void {
    ...
    if (input?.references?.length && !process.env.OPENROUTER_IMAGE_API_KEY) {
      throw new AppError("AGENT_IMAGE_GENERATION_CONFIG_INVALID",
        "Редактирование фотографий требует настроенного OpenRouter image provider");
    }
  },
