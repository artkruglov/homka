...
async function checkedWorkspaceDirectory(root: string, workspaceId: string): Promise<string> {
  const directory = workspaceDirectory(root, workspaceId);
  let metadata;
  try { metadata = await lstat(directory); } catch (error) {
    // Workspaces are created lazily. Callers retain their existing missing-file/list/write policy.
  ...
  if (!metadata.isDirectory()) {
    throw new AppError("AGENT_WORKSPACE_PATH_INVALID", "Корень workspace должен быть каталогом");
  }
  return directory;
}
...
async function physicalPath(
  ...
  const directory = await checkedWorkspaceDirectory(root, workspaceId);
...
export async function listWorkspaceStoredFiles(
  ...
  const files = await scanWorkspaceDirectory(await checkedWorkspaceDirectory(root, workspaceId));
