  ...
  try {
    ...
    const existing = await client.query<{
      ...
      `SELECT alias.entry_id, message.actor_kind, message.sequence_id::text
        ...
        WHERE alias.group_id = $1 AND alias.telegram_chat_id =
           (SELECT telegram_chat_id FROM telegram_groups WHERE id = $1) AND alias.telegram_message_id = ANY($2::bigint[])`,
    ...
    const inserted = await client.query<{ id: string }>(
      ...
      [input.groupId, sequenceId, AGENT_ACTOR_ID, messageIds[0], input.messageThreadId,
        readTelegramAgentIdentity().name, input.contentText, input.replyToEntryId, input.deliveredAt,
