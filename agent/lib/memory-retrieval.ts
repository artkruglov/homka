...
/**
 * Placed after the used-memory reminder whenever records were shown and the chat has search.
 * 14 September 2026, SEARCH-14F-R4: with a record about the museum trip the agent answered twice
 * without a web_search. `stress/search-evals/freshness.ts` with thinking disabled as in production:
 * search with a stale record 7/12 without this reminder, 12/12 with it (11/12 together with the
 * WEB_SEARCH_RULES sentence), a page opened 5/12 without it and 8/12 with it.
 */
export const MEMORY_FRESHNESS_REMINDER =
  "Записи памяти и прежние ответы не подтверждают текущие часы, цены, расписания и правила: для такого вопроса снова вызови `web_search` и открой первоисточник через `web_fetch`.";
