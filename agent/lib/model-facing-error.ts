...
export function normalizeModelFacingError(
  ...
  // Eve 0.40 web_fetch throws this exact, status-only error for a remote HTTP failure.
  // Recognize only that shape and that read-only tool; never expose an arbitrary message.
  const webStatus = context.toolName === "web_fetch" && error instanceof Error
    ? /^Request failed with status code: ([45][0-9]{2})$/u.exec(error.message)?.[1]
    : undefined;
  if (webStatus) {
    return new ModelFacingError({
      category: "dependency",
      code: "AGENT_WEB_FETCH_HTTP_ERROR",
      reason: `Сайт вернул HTTP ${webStatus}; содержимое страницы не получено.`,
      correction: "Сообщите об ответе сайта, не называйте его внутренним сбоем бота и не пересказывайте непрочитанное. Не повторяйте этот запрос автоматически и не обходите запрет сайта. Для ответа можно искать другой доступный первоисточник через разрешённый web_search; шаги инструкции без подтверждения не добавляйте.",
      retryable: false,
