...
describeWithDatabase("session repository", () => {
  ...
  it("defers a requested rotation until the pending operation completes", async () => {
    ...
    await sessionRepository.requestRotation(current.id, "user_requested");
  ...
  it("keeps a failure as the rotation reason when a new context is requested afterwards", async () => {
    const f = await fixture();
    const current = await sessionRepository.prepareTurn({
      baseContinuationToken: "104::",
      kind: "canonical",
    ...
    await sessionRepository.requestRotation(current.id, "history_unrecoverable");
    await sessionRepository.requestRotation(current.id, "user_requested");
    await expect(database().query(
      "SELECT rotation_reason FROM conversation_sessions WHERE id = $1",
      [current.id],
    )).resolves.toMatchObject({ rows: [{ rotation_reason: "history_unrecoverable" }] });
  });

  it("finds the live application session of an Eve session only while it is not retired", async () => {
    const f = await fixture();
    const current = await sessionRepository.prepareTurn({
      baseContinuationToken: "105::",
      kind: "canonical",
    ...
    await expect(sessionRepository.findActiveIdByEveSessionId("wrun_lost_boundary")).resolves.toBeNull();
    await sessionRepository.bindEveSession(current.id, "wrun_lost_boundary");
    await expect(sessionRepository.findActiveIdByEveSessionId("wrun_lost_boundary")).resolves.toBe(current.id);

    // A lost turn boundary rotates the session with the failure reason the owner digest counts.
    await sessionRepository.requestRotation(current.id, "session_failed");
    await expect(database().query(
      "SELECT rotation_reason FROM conversation_sessions WHERE id = $1 AND rotation_requested_at IS NOT NULL",
      [current.id],
    )).resolves.toMatchObject({ rows: [{ rotation_reason: "session_failed" }] });
  });
  ...
  it("leases only retired sessions whose one-day retention has elapsed", async () => {
    ...
    // Upstream 63d7e65: a failed deletion waits out its cooldown and is then tried again.
