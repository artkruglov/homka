 ...
 * - `SessionRotationReason`: why the next message starts a new session; the owner digest counts failures.
 */
...
export type SessionRotationReason = "history_unrecoverable" | "session_failed" | "user_requested";

const FAILURE_ROTATION_REASONS_SQL = "'session_failed', 'history_unrecoverable'";
...
export const sessionLifecycleEventRepository = {
      ...
      const result = await client.query(
        `UPDATE conversation_sessions
             SET pending_operation = false,
                 ...
                 rotation_reason = CASE WHEN kind = 'canonical' THEN 'session_failed' ELSE rotation_reason END,
                 eve_session_id = $2,
  ...
  /** The live application session bound to an Eve session, or null once retired or never bound. */
  async findActiveIdByEveSessionId(eveSessionId: string): Promise<string | null> {
    const result = await database().query<{ id: string }>(
      `SELECT id FROM conversation_sessions
        WHERE eve_session_id = $1 AND retired_at IS NULL`,
      [eveSessionId],
    );
    return result.rows[0]?.id ?? null;
  },

  async requestRotation(id: string, reason: SessionRotationReason): Promise<void> {
    // A failure stays the reason when a new context is requested before the next message.
    const result = await database().query(
      `UPDATE conversation_sessions
          SET rotation_requested_at = now(),
              rotation_reason = CASE WHEN rotation_reason IN (${FAILURE_ROTATION_REASONS_SQL})
                THEN rotation_reason ELSE $2 END
        WHERE id = $1 AND retired_at IS NULL`,
      [id, reason],
    );
    if (result.rowCount !== 1) {
