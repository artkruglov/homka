...
import { resolveSessionSpacePolicy, sessionSpaceChanged, type SessionSpacePolicy } from "./session-space-context.js";
...
export interface PrepareSessionInput {
  /** Verified application selection; never populated from model arguments. */
...
export interface PreparedSession {
  spacePolicy?: SessionSpacePolicy;
...
async function createInitialSession(
  ...
  space: SessionSpacePolicy | null = null,
): Promise<SessionRow> {
  ...
  const result = await client.query<SessionRow>(
    `INSERT INTO conversation_sessions
       (thread_id, generation, family_id, owner_user_id, group_id, scope, kind, task_state,
         telegram_forum_topic_id, conversation_key, continuation_token, started_at, last_activity_at, space_id, space_policy_version)
  ...
  const result = await client.query<{ thread_id: string }>(
    `SELECT thread_id
      ...
      WHERE group_id = $1
        ...
        AND space_id IS NOT DISTINCT FROM $3::uuid
        AND space_policy_version IS NOT DISTINCT FROM $4::integer
      ORDER BY (kind = 'task' AND retired_at IS NULL) DESC, generation DESC, started_at DESC
      LIMIT 1
      FOR UPDATE`,
    [input.groupId, input.telegramForumTopicId, space?.spaceId ?? null, space?.policyVersion ?? null],
  );
...
async function initialGeneration(client: PoolClient, baseToken: string): Promise<number> {
  ...
  const result = await client.query<{ next_generation: number }>(
    `SELECT greatest(
       coalesce((SELECT next_generation FROM conversation_route_generations WHERE route_owner=$1),0),
       coalesce((SELECT max(generation)+1 FROM conversation_sessions WHERE conversation_key=$2),0)
     ) AS next_generation`,
    [routeOwner, baseToken],
  );
...
export const sessionRepository = {
  ...
  async prepareTurn(input: PrepareSessionInput): Promise<PreparedSession> {
    ...
    try {
      ...
      // Lock the audience before session/route rows so revocation cannot race preparation.
      const space = await resolveSessionSpacePolicy(client, input);
      ...
      if (current && !space) sessionSpaceChanged(current, null);
      ...
      if (!current) {
        ...
        const replacement = await canonicalReplacementSeed(client, input, space);
        ...
        current = await createInitialSession(client, input, generation, replacement?.threadId, space);
        if (replacement) {
          await client.query(
            `INSERT INTO audit_events (family_id, event_type, subject_id, metadata)
      ...
      if (sessionSpaceChanged(current, space)) {
        if (input.kind === "task") {
          throw new AppError("AGENT_SESSION_SPACE_CONTEXT_STALE", "Состав пространства изменился. Прежнее подтверждение больше не продолжает диалог");
        }
        await client.query(`UPDATE conversation_sessions SET retired_at=$2, delete_after=$3,
          pending_operation=false WHERE id=$1`, [current.id, input.now,
          new Date(input.now.getTime() + SESSION_RETENTION_DAYS * MILLISECONDS_PER_DAY)]);
        // Old Telegram anchors must not carry old summaries/tool results into the new audience.
        await client.query("DELETE FROM conversation_session_routes WHERE session_id=$1", [current.id]);
        current = await createInitialSession(client, input,
          await nextThreadGeneration(client, current.thread_id), undefined, space);
        trustZoneRecreated = true;
      }
