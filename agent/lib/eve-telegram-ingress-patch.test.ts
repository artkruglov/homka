...
describe("Eve Telegram verified ingress patch", () => {
  ...
  it("fails fast when the pinned Telegram runtime artifact does not match", async () => {
    ...
    try {
      ...
      // The PostgreSQL connection recovery patch runs next and also pins graphile-worker.
      await cp(resolve("node_modules/graphile-worker"), join(root, "node_modules/graphile-worker"), { recursive: true });
