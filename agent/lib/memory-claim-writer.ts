...
import { lockSlot, supersedeSlotClaims, type SlotIdentity } from "./memory-slot-supersede.js";
...
/**
 * Право писать в область проверяется первым оператором транзакции: порядок блокировок требует,
 * чтобы родительское пространство бралось раньше живых проверок членства и группы.
 */
...
async function replayBeforeTitleEmbedding(
  ...
  try {
    ...
    await requireSpaceWriteAccess(client, auth);
...
export async function createMemoryClaim(
  ...
  try {
    ...
    const { scopePartitionKey, spaceId } = resolveClaimPartition(auth, input.scope);
    const contentNormalized = prepared?.contentNormalized ?? normalizeMemoryClaimContent(input.content);
    const slot: SlotIdentity | null = input.attribute === undefined ? null : {
      ...
      subjectLabel: prepared?.subjectLabel ?? null, subjectParticipantId: prepared?.subjectParticipantId ?? null,
      subjectUserId: prepared?.subjectUserId ?? null,
    };
    if (slot) await lockSlot(client, auth, slot);
      ...
      `INSERT INTO memory_items
         (family_id, owner_user_id, group_id, author_user_id, author_telegram_user_id,
          ...
          sensitivity, operation_key, origin_conversation_id, subject_participant_id,
           ...
           claim_status, duplicate_of, attribute, occurred_at, space_id)
    ...
    if (slot) {
      await supersedeSlotClaims(client, auth, { ...slot, newClaimId: row.id, systemActor: input.systemActor === true });
    }
