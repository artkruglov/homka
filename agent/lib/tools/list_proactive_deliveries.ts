...
function requireDeliveryAuthorization(
  ...
  // Область берётся из того же проверенного хода; её свежесть проверяет общая граница инструментов.
  const space = authorization.space ? { space: authorization.space } : {};
