...
function dependencies() {
  return {
    readAttachment: vi.fn().mockResolvedValue(Buffer.from(
      'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Wl6D1sAAAAASUVORK5CYII=', 'base64')),
    client: {
    ...
    workspaces: {
      readBinary: vi.fn().mockResolvedValue({bytes: Buffer.from(
        'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Wl6D1sAAAAASUVORK5CYII=', 'base64'), file: FILE, workspaceId: 'workspace-1'}),
...
describe("generate_image", () => {
  it("edits a journal photo through the authorized attachment reader without importing it", async () => {
    const deps=dependencies();
    const id='7efc0d62-c01e-46d4-8ead-e355aa438998';
    await createGenerateImageTool(deps as never).execute({...INPUT,referenceAttachmentIds:[id]},context());
    expect(deps.readAttachment).toHaveBeenCalledWith(expect.objectContaining({groupId:'group-1'}),id);
    expect(deps.workspaces.readBinary).not.toHaveBeenCalled();
    expect(deps.client.generate).toHaveBeenCalledWith(expect.objectContaining({references:[{bytes:expect.any(Buffer),mediaType:'image/png'}]}));
  });
  it("reads edit sources only in the current authorized workspace and hashes their content", async () => {
    const deps = dependencies();
    const tool = createGenerateImageTool(deps as never);
    await tool.execute({...INPUT, referencePaths:['photos/source.png']}, context());
    expect(deps.workspaces.readBinary).toHaveBeenCalledWith(expect.objectContaining({groupId:'group-1'}), 'group', 'photos/source.png');
    expect(deps.client.generate).toHaveBeenCalledWith(expect.objectContaining({references:[{
      bytes: expect.any(Buffer), mediaType:'image/png',
    }]}));
    const originalHash = deps.operations.begin.mock.calls[0]![0].inputHash;
    const bytes = Buffer.concat([(await deps.workspaces.readBinary()).bytes, Buffer.from('changed')]);
    deps.workspaces.readBinary.mockResolvedValue({bytes,file:FILE,workspaceId:'workspace-1'});
    await tool.execute({...INPUT, referencePaths:['photos/source.png']}, context());
    expect(deps.operations.begin.mock.calls[1]![0].inputHash).not.toBe(originalHash);
  });

  it.each(['denied','invalid'])('never reserves or charges for an %s edit source', async kind => {
    const deps = dependencies();
    if (kind === 'denied') deps.workspaces.readBinary.mockRejectedValue(new Error('access denied'));
    else deps.workspaces.readBinary.mockResolvedValue({bytes:Buffer.from('<svg/>'),file:FILE,workspaceId:'workspace-1'});
    await expect(createGenerateImageTool(deps as never).execute({...INPUT, referencePaths:['photos/source.png']},context())).rejects.toThrow();
