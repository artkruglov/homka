...
import readSharedTasks from "../tools/read_shared_tasks.js";
...
const EXTERNAL_DIRECT_TOOLS: Readonly<Record<DirectExternalToolName, AnyToolDefinition>> = {
  generate_video:generateVideo as AnyToolDefinition,
...
function buildExternalToolSurface(
  ...
  const videoGenerationAllowed=VIDEO_GENERATION_AVAILABLE&&!scheduledRun&&allowed.has('generate_video');
  ...
  if(videoGenerationAllowed){
    const wait=sleep() as AnyToolDefinition;
    surface.sleep=defineTool({...wait,async execute(input,ctx){
      return withExternalGroupCapability(ctx,'generate_video',async()=>wait.execute(input,ctx));
    }}) as AnyToolDefinition;
  }
  // Granted application capabilities are re-checked at execution against the live policy.
  for (const capability of allowed) {
    if (scheduledRun && capability === "manage_shared_tasks") continue;
    ...
    if(capability==='generate_video'&&!videoGenerationAllowed)continue;
...
/**
 * Инструменты, которых нет ни у фонового хода, ни у сабагента.
 *
 * Общее у них одно: рядом нет человека, который подтвердит, поправит или остановит действие.
 * Поэтому сюда попадает всё, что говорит от его имени наружу (`send_to_chat`, `publish_memory`),
 * меняет рамку следующего хода (`manage_space`, `manage_behavior_preference`), принимает
 * смысловое решение о памяти (`remember`) или создаёт заказ у продавца (`grocery_cart`).
 * Чтение остаётся: фоновому обзору оно и нужно.
 */
const BACKGROUND_DENIED_TOOLS = [
  "generate_video",
  "sleep",
  "manage_joint_decision",
  "manage_errand",
  "grocery_cart",
...
/**
 * Обзор дня собирается из дел, а планировщика у фонового хода нет. Чтение без права менять это и
 * есть требование W18: фоновый агент читает задачи для обзора и не может их трогать.
 */
const BACKGROUND_ONLY_TOOLS: ToolMap = wrapModelFacingToolMap({
  read_shared_tasks: readSharedTasks as unknown as AnyToolDefinition,
});

function backgroundSurface(surface: ToolMap): ToolMap {
  const denied = new Set<string>(BACKGROUND_DENIED_TOOLS);
  return {
    ...Object.fromEntries(Object.entries(surface).filter(([name]) => !denied.has(name))),
    ...BACKGROUND_ONLY_TOOLS,
  };
}
...
export function buildSubagentToolSurface(input: ModeToolSurfaceInput): ToolMap {
  const effectiveInput = input.environment === "external"
    ? {
      ...input,
      capabilities: new Set([...input.capabilities].filter((name) => name !== "generate_image"&&name!=='generate_video')),
    }
    : input;
  // Сабагент к тому же не рисует: право на изображение принадлежит корневому ходу.
  const { generate_image: _generateImage, ...rest } = buildModeToolSurface(effectiveInput);
  const surface = backgroundSurface(rest);
