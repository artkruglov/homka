...
export const EXTERNAL_GROUP_CAPABILITY_CATALOG = [
  {name:'generate_video',usage:'создать видео Seedance 2.5 из текста или фото текущей группы, проверить его готовность и отправить MP4; общий лимит $30 на человека в месяц'},
  { name: "manage_shared_tasks", usage: "предлагать и принимать задачи текущей группы, читать их список и менять статус с проверкой исполнителя" },
  {
    name: "generate_image",
  ...
  {
    name: "web_search",
    usage: "искать в интернете провайдерским поиском модели; выполняется на стороне провайдера, отзыв права действует со следующего шага модели",
  },
