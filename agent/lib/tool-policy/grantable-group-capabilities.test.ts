...
vi.mock('../video-generation/video-generation-availability.js',()=>({VIDEO_GENERATION_AVAILABLE:false}));
...
describe("grantable external group capabilities without a Codex subscription", () => {
  ...
  it("keeps every other catalog capability grantable", () => {
    expect([...GRANTABLE_EXTERNAL_GROUP_TOOL_NAMES]).toEqual(
      EXTERNAL_GROUP_TOOL_NAMES.filter((name) => !isSubscriptionOnlyExternalGroupToolName(name)&&name!=='generate_video'),
    );
    expect(GRANTABLE_EXTERNAL_GROUP_TOOL_NAMES.length).toBeGreaterThan(0);
  });
  it('keeps an unavailable video grant parseable without making it grantable',()=>{
    expect(isGrantableExternalGroupToolName('generate_video')).toBe(false);
    expect(parseExternalGroupToolAllowlist(['generate_video','remember'])).toEqual(new Set(['generate_video','remember']));
  });
