...
import { memorySpaceParameters, type MemoryAuthorization, MemoryScope } from "./memory-context.js";
  ...
  const result = await client.query<ClaimRow>(
    `SELECT claim.id, ref.memory_ref, claim.content, claim.kind, claim.attribute, claim.confirmation,
     ...
     WHERE claim.family_id = $1
       ...
       AND ${spaceReadClause({
         alias: "claim",
         parameters: { family: "$1", group: "$5", spaceId: "$7", user: "$3", version: "$8" },
       })}
     ORDER BY claim.id`,
    [auth.familyId, conversation.scope, auth.userId, subjectUserIds, auth.groupId,
      subjectParticipantIds, ...memorySpaceParameters(auth)],
  );
  return result.rows;
}
