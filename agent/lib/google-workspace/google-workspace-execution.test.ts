...
function approval(argv: string[], scheduled = false) {
  return (executeGoogleWorkspace as unknown as {
    approval: (context: unknown) => unknown;
  }).approval({ toolInput: { argv }, session: { auth: {
    current: { attributes: scheduled ? { scheduledRunId: "scheduled-test" } : {} }, initiator: null,
  } } });
}
...
describe("execute_google_workspace approval", () => {
  it("denies background mutation immediately while preserving reads", () => {
    expect(approval(["calendar", "events", "list", "--params", "{}"], true)).toBe("not-applicable");
    expect(approval(["calendar", "events", "insert", "--json", "{}"], true)).toMatchObject({
      type: "denied", reason: expect.stringContaining("AGENT_GOOGLE_BACKGROUND_MUTATION_DENIED"),
    });
  });
  it("rejects a direct background mutation before credential execution", async () => {
    await expect((executeGoogleWorkspace as unknown as {
      execute: (input: unknown, ctx: unknown) => Promise<unknown>;
    }).execute({ argv: ["calendar", "events", "insert", "--json", "{}"] }, {
      session: { auth: { current: { attributes: { scheduledRunId: "scheduled-test" } } } },
    })).rejects.toThrow(/AGENT_GOOGLE_BACKGROUND_MUTATION_DENIED/u);
  });
