...
describe("Google Workspace profile store", () => {
  it.each(["exists", "write", "remove"] as const)("rejects %s through another profile's linked root", async (operation) => {
    const root = await mkdtemp(join(tmpdir(), "osinara-gws-root-"));
    roots.push(root);
    const store = createGoogleWorkspaceProfileStore(root);
    const credentials = { client_id: "test-id", client_secret: "test-secret", refresh_token: "test-refresh", type: "authorized_user" as const };
    await store.write(FAMILY_WORKSPACE_ID, credentials);
    await symlink(join(root, FAMILY_WORKSPACE_ID), join(root, PERSONAL_WORKSPACE_ID));
    await expect(operation === "write" ? store.write(PERSONAL_WORKSPACE_ID, { ...credentials, refresh_token: "changed" }) : store[operation](PERSONAL_WORKSPACE_ID))
      .rejects.toMatchObject({ code: "AGENT_GOOGLE_WORKSPACE_PROFILE_PATH_INVALID" });
    expect(JSON.parse(await readFile(join(root, FAMILY_WORKSPACE_ID, "credentials.json"), "utf8"))).toEqual(credentials);
  });
