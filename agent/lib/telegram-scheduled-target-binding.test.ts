 ...
 * - Приостановленный по неподтверждённой аудитории запуск закрывается сам: иначе его строка
 *   осталась бы `running`, аренда расписания — вечной, и следующее вхождение не наступило бы.
 */
...
vi.mock("./telegram-final-delivery.js", () => ({
  ...
  suspendTelegramFinalOutput: dependencies.suspendFinalOutput,
}));
vi.mock("./spaces/space-delivery-authorization.js", () => ({
  authorizeSpaceDelivery: dependencies.spaceDelivery,
  notifySuspendedDelivery: vi.fn(),
  spaceDeliveryRequestFromAuth: vi.fn(() => ({
    chatType: "supergroup", familyId: "family-1", groupId: "group-1", now: new Date(), userId: null,
  })),
}));
...
describe("scheduled Telegram target binding", () => {
  beforeEach(() => {
    ...
    dependencies.spaceDelivery.mockResolvedValue({ allowed: true });
  });

  it("closes a scheduled run whose chat audience is not proved instead of leaving it running", async () => {
    const handler = dependencies.channelConfig?.events?.["message.completed"];
    dependencies.spaceDelivery.mockResolvedValue({
    ...
    expect(dependencies.suspendFinalOutput).toHaveBeenCalledWith(expect.objectContaining({
      diagnosticCode: "AGENT_SPACE_DELIVERY_AUDIENCE_UNPROVEN",
    }));
    expect(dependencies.failRun).toHaveBeenCalledWith(
      "application-session-1", "eve-session-1", "AGENT_SPACE_DELIVERY_AUDIENCE_UNPROVEN",
      expect.any(Date),
    );
  });
