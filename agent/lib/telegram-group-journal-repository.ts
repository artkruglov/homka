  ...
  async findSeriesSequences(input) {
    ...
    const result = await database().query<{ sequence_id: string }>(
      `SELECT sequence_id::text
       ...
       WHERE conversation_id = $1 AND telegram_chat_id =
         (SELECT telegram_chat_id FROM application_conversations WHERE id = $1) AND actor_id = $2 AND telegram_message_id = ANY($3::bigint[])
      ...
      const duplicate = await client.query<{
        ...
        `SELECT alias.entry_id, message.sequence_id::text,
         ...
         WHERE alias.group_id = $1 AND alias.telegram_chat_id =
           (SELECT telegram_chat_id FROM telegram_groups WHERE id = $1) AND alias.telegram_message_id = $2`,
        [groupId, messageId],
      );
      if (duplicate.rows[0]) {
