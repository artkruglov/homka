...
describe("createConfiguredLanguageModel", () => {
  ...
  it.each([false, true])("explicitly disables Anthropic thinking on every tool step (inherited override: %s)", async (override) => {
    const bodies: Record<string, unknown>[] = [];
    const model = createConfiguredLanguageModel({
      apiKey: "test-key", modelId: "deepseek-v4-flash", maxOutputTokens: 1000,
      transport: { protocol: "anthropic-messages", baseUrl: "https://api.deepseek.com/anthropic",
        authentication: "api-key", reasoning: { type: "none" } },
      fetch: async (_input, init) => {
        bodies.push(JSON.parse(String(init?.body)));
        return jsonResponse({ id: "msg_qa", type: "message", role: "assistant", model: "deepseek-v4-flash",
          content: bodies.length === 1 ? [{ type: "tool_use", id: "call_qa", name: "probe", input: {} }]
            : [{ type: "text", text: "done" }],
          stop_reason: bodies.length === 1 ? "tool_use" : "end_turn", stop_sequence: null,
          usage: { input_tokens: 5, output_tokens: 5 } });
      },
    });
    const result = await generateText({ model, prompt: "probe", maxRetries: 0,
      tools: { probe: tool({ inputSchema: z.object({}), execute: async () => "ok" }) },
      stopWhen: stepCountIs(2),
      ...(override ? { providerOptions: { anthropic: { thinking: { type: "adaptive" } } } } : {}),
    });
