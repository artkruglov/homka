...
describeWithDatabase("memory slots", () => {
  ...
  it("serializes concurrent first writes to one empty slot so exactly one claim stays active", async () => {
    // Upstream 960c4b9: row locks alone miss the first-write race. Here the scope quota lock also
    // serializes these writers, so this guards the slot lock against losing either of them.
    const fixture = await createMainAgentMemoryFixture();
    ...
    const active = await database().query<{ id: string }>(
      "SELECT id FROM memory_items WHERE id = ANY($1::uuid[]) AND claim_status = 'active'",
      [written.map((memory) => memory.id)],
    );
    expect(active.rowCount).toBe(1);
  });

  it("does not retire a project-bound claim from another slot identity", async () => {
    const fixture = await createMainAgentMemoryFixture();
    const labelled = (content: string, operationKey: string): CreateMemoryInput => ({
      ...slotInput(fixture, content, operationKey),
      attribute: "срок",
      explicitSource: {
        conversationId: fixture.conversationId,
        subject: { kind: "label", label: "Ремонт" },
        timelineEntryId: fixture.timelineEntryId,
      },
      kind: "fact",
    });
    const projectBound = await memoryRepository.create(fixture.auth, labelled("Ремонт закончат к октябрю", "project-1"));
    const project = await database().query<{ id: string }>(
      `INSERT INTO memory_projects (family_id, scope, scope_partition_key, title)
       VALUES ($1, 'family', $1, 'Дача') RETURNING id`,
      [fixture.familyId],
    );
    await database().query("UPDATE memory_items SET memory_project_id = $1 WHERE id = $2", [project.rows[0]!.id, projectBound.id]);

    await memoryRepository.create(fixture.auth, labelled("Ремонт закончат к ноябрю", "project-2"));

    await expect(database().query("SELECT claim_status::text FROM memory_items_all WHERE id = $1", [projectBound.id]))
      .resolves.toMatchObject({ rows: [{ claim_status: "active" }] });
  });
