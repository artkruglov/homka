 ...
 * - `memoryRetention`: R = exp(-age / S); age from creation or the later last reinforcement.
...
export function memoryRetention(input: MemoryRetentionInput, now: Date): number {
  // An old event learned today is new knowledge (upstream 960c4b9); occurredAt serves only the
  // event-date window of search. Reinforcement never precedes creation, so the later one wins.
