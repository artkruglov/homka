...
async function loadReviewMemoryContext(client: PoolClient, input: {
  ...
  /** Область пакета; `null`, пока режим семьи прежний. */
  spaceId: string | null;
  ...
  async claimPending(input: {
    ...
    try {
      ...
      for (const row of claimed.rows) {
        ...
        // Блок уже сохранённых записей уходит в промпт целиком, поэтому он ограничивается той же
        // областью, что и сам ход проверки: иначе в него попадут записи соседней области.
        const reviewSpace = await resolveTurnSpaceContext(client, {
          chatType: row.group_id === null ? "private" : "supergroup",
          familyId: row.family_id,
          groupId: row.group_id,
          userId: row.sponsor_user_id,
        });
        const existing = await loadReviewMemoryContext(client, {
          ...
          spaceId: reviewSpace?.spaceId ?? null,
        });
        claims.push({
