...
function alertText(alert: MemoryReviewOwnerAlertClaim): string {
  ...
  if (alert.diagnosticCode === "AGENT_MEMORY_REVIEW_PASS_SKIPPED") {
    ...
    return [
      ...
      `${readTelegramAgentIdentity().name} оборвала проверку памяти группы «${groupTitle}» на середине.`,
      `Сообщения ${alert.fromSequence}–${alert.throughSequence} остались непроверенными, ` +
        "и автоматически к ним никто не вернётся.",
      `Если в них было важное, скажите об этом в чате, и ${readTelegramAgentIdentity().name} запомнит заново.`,
    ].join("\n\n");
  }
  return [
    "AGENT_MEMORY_REVIEW_BLOCKED",
    `${readTelegramAgentIdentity().name} остановила автоматическую проверку памяти группы «${groupTitle}».`,
