/**
 * Migration 098 stuck-lane healing integration tests.
 *
 * Constructs covered:
...
describeWithDatabase("migration 098: heal stuck memory review lanes", () => {
  beforeEach(async () => {
  ...
  it("skips a dead head with finished successors and moves the cursor; leaves a lone head alone", async () => {
    ...
    // One group on 3 September: ambiguous head @5 (message 6), completed @6..20 and @20..30 behind it.
    ...
    await database().query(await readFile(resolve("migrations", "098_memory_review_heal_stuck_lanes.sql"), "utf8"));
