...
describeWithDatabase("memoryRetrievalRepository", () => {
  ...
  it("does not fade an old event learned today, exactly as the TypeScript formula", async () => {
    const learned = await database().query<{ id: string }>(
      `INSERT INTO memory_items
         (family_id, owner_user_id, author_user_id, author_telegram_user_id, scope, kind, content, source,
          confirmation, sensitivity, operation_key, embedding_status, occurred_at)
       VALUES ($1, $2, $2, $3, 'personal', 'episode', 'Анна в детстве жила в Туле', 'test:old-event',
               'model_high', 'normal', 'old-event', 'indexed', now() - interval '400 days')
       RETURNING id`,
      [auth.familyId, auth.userId, auth.telegramActorId],
    );
    await database().query(
      `INSERT INTO memory_embedding_chunks
         (memory_item_id, chunk_index, content, start_offset, end_offset, embedding, embedding_model)
       VALUES ($1, 0, 'Анна в детстве жила в Туле', 0, 26, $2::vector, $3)`,
      [learned.rows[0]!.id, `[${vector(0, 1).join(",")}]`, MEMORY_EMBEDDING_MODEL_VERSION],
    );

    const results = await memoryRetrievalRepository.search(auth, "Тула", vector(0, 1));
    const found = results.find((result) => result.memory.id === learned.rows[0]!.id)!;

    expect(found.retention).toBeCloseTo(memoryRetention({
      attribute: null,
      createdAt: new Date(),
      kind: "episode",
      lastReinforcedAt: null,
    ...
    expect(found.retention).toBeGreaterThan(0.95);
  });
