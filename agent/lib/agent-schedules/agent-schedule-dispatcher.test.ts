 ...
 * - A dropped database connection after Eve accepted the run never fails or discards that run.
 */
...
function preparedSession(id: string) {
  return { continuationToken: "101::schedule:run-1", generation: 0, id, rotated: false, sandboxSessionId: `${id}-sandbox` };
}
...
describe("agent schedule dispatcher", () => {
  ...
  it.each([
    ["recovers", 1],
    ["keeps the started run when recovery is exhausted", 10],
  ] as const)("%s after the running marker loses its database connection", async (_name, drops) => {
    vi.useFakeTimers();
    const dropped = new Error("Connection terminated unexpectedly");
    const markRunning = vi.fn();
    for (let index = 0; index < drops; index += 1) markRunning.mockRejectedValueOnce(dropped);
    const repository = {
    ...
    const send = vi.fn().mockResolvedValue({ continuationToken: "t", getEventStream: vi.fn(), id: "eve-session-1" });

    const dispatched = createAgentScheduleDispatcher({
      discardSession,
      prepareHistory: vi.fn(),
      prepareSession: vi.fn().mockResolvedValue(preparedSession("app-session-1")),
      repository,
    ...
    if (drops === 1) expect(markRunning).toHaveBeenCalledTimes(2);
  ...
  it("carries the space its prepared session proved into the scheduled run", async () => {
    const repository = {
      claimDue: vi.fn().mockResolvedValue([job]),
      failClaim: vi.fn(),
      markDispatchStarted: vi.fn(),
    ...
    const prepareSession = vi.fn().mockResolvedValue({
      ...
      rotated: false,
      sandboxSessionId: "sandbox-1",
      spacePolicy: { policyVersion: 4, spaceId: "22222222-2222-4222-8222-222222222222" },
    });
    const send = vi.fn().mockResolvedValue({
      continuationToken: "101::schedule:run-1",
      getEventStream: vi.fn(),
    ...
    // Без этих атрибутов общая граница инструментов не увидит области и после включения
    // режима остановит фоновый ход целиком.
    expect(send).toHaveBeenCalledWith(expect.any(String), {
      auth: expect.objectContaining({
        attributes: expect.objectContaining({
          spaceId: "22222222-2222-4222-8222-222222222222",
          spacePolicyVersion: "4",
        }),
      }),
    });
  });
