...
describeWithDatabase("agent schedule repositories", () => {
  ...
  it("claims a weekday schedule once and advances it after Eve completion", async () => {
    ...
    // A retry after a lost acknowledgement sees its own running marker and succeeds; another
    // Eve session is still a stale lease (upstream 2167e2c).
    await expect(agentScheduleDispatchRepository.markRunning(claimed!, {
      applicationSessionId: prepared.id,
      eveSessionId: "eve-schedule-1",
    })).resolves.toBeUndefined();
    await expect(agentScheduleDispatchRepository.markRunning(claimed!, {
      applicationSessionId: prepared.id,
      eveSessionId: "eve-schedule-other",
    })).rejects.toMatchObject({ code: "AGENT_SCHEDULE_LEASE_STALE" });
    const delivery = {
      applicationSessionId: prepared.id,
  ...
  it("does not auto-resume a migrated schedule after an explicit user pause",async()=>{
    const f=await createFixture();const auth=familyAuth(f,'owner');
    const schedule=await agentScheduleRepository.create(auth,{scope:'family',timezone:'UTC',title:'Overview',
      userRequest:'Overview',scenarioPrompt:'Overview',firstRunAt:new Date('2026-10-01T12:00:00Z'),
      recurrence:{kind:'once'},operationKey:'migration-create'});
    await database().query("UPDATE agent_schedules SET status='paused',last_error_code='AGENT_TELEGRAM_GROUP_MIGRATED' WHERE id=$1",[schedule.id]);
    await agentScheduleRepository.update(auth,schedule.id,{enabled:false,operationKey:'manual-pause-after-migration'});
    expect((await database().query('SELECT status,last_error_code FROM agent_schedules WHERE id=$1',[schedule.id])).rows[0])
