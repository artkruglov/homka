...
/**
 * Оговорка области для строки под псевдонимом `schedule`: $3 человек, $4 область, $5 версия,
 * $6 группа.
 *
 * Автоматизация внешней группы исключена намеренно: её заводят из лички владельца, а область ей
 * даёт привязка самого чата — ход её не доказывает и доказать не может. Аудиторию такой строки
 * проверяет доставка (`space-delivery-authorization.ts`) перед каждой отправкой.
 */
export const AGENT_SCHEDULE_SPACE_CLAUSE = `(schedule.scope = 'group' OR ${spaceReadClause({
  alias: "schedule",
...
export async function selectAgentSchedule(
  client: PoolClient,
  auth: Pick<AgentScheduleAuthorization, "familyId" | "groupId" | "space" | "userId">,
  id: string,
  lock = false,
  ...
  const result = await client.query<MutableAgentScheduleRow>(
    `SELECT ${AGENT_SCHEDULE_COLUMNS}, family_id, owner_user_id, author_user_id,
      ...
      WHERE schedule.family_id = $1 AND schedule.id = $2
        AND ${AGENT_SCHEDULE_SPACE_CLAUSE}${lock ? " FOR UPDATE" : ""}`,
