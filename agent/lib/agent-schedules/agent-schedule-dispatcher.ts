...
import { resolveTurnSpace } from "../spaces/turn-space-selection.js";
...
function scheduledAuth(job: ClaimedAgentSchedule, prepared: PreparedSession) {
  return {
    attributes: {
      ...
      ...(prepared.spacePolicy === undefined ? {} : {
        spaceId: prepared.spacePolicy.spaceId,
        spacePolicyVersion: String(prepared.spacePolicy.policyVersion),
      }),
...
async function dispatchOne(dependencies: AgentScheduleDispatcherDependencies, job: ClaimedAgentSchedule, now: Date): Promise<void> {
  ...
  try {
    ...
    handedOff = true;
    // Eve owns the run now: a dropped connection retries only the marker, which recognizes its own
    // earlier commit (upstream 2167e2c). Failing the claim here would stop a live run's schedule.
    await recoverDatabaseBookkeeping(() => dependencies.repository.markRunning(job, {
      applicationSessionId: prepared.id,
      eveSessionId: session.id,
    }));
  } catch (error) {
    // The dispatching lease then expires into the ambiguous path, which never starts it again.
    if (handedOff && isTransientDatabaseConnectionError(error)) throw error;
    console.error(
      JSON.stringify({
...
export function dispatchDueAgentSchedules(to: ScheduleToFn, now = new Date()): Promise<number> {
  return createAgentScheduleDispatcher({
    ...
    prepareSession: async (job, continuationToken, currentTime) => {
      // Фоновый ход тоже принадлежит области: без неё после включения режима он падает
      // на подготовке сессии, а его инструменты — на общей границе.
      const spaceContext = await resolveTurnSpace({
