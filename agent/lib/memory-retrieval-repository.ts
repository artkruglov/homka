...
/** Позиции области различаются: у поиска заняты первые двадцать, у замыкания конфликтов — пять. */
const SEARCH_SPACE_PARAMETERS = { id: "$21", version: "$22" } as const;
const CLOSURE_SPACE_PARAMETERS = { id: "$6", version: "$7" } as const;

function authorizedClaimPredicate(
  alias: "a" | "b" | "item" | "partner",
  spaceParameters: { id: string; version: string },
): string {
      ...
      `WITH authorized AS NOT MATERIALIZED (
          ...
          JOIN memory_item_refs AS ref ON ref.memory_item_id = item.id
           WHERE item.family_id = $1 AND item.claim_status = 'active'
             AND ${authorizedClaimPredicate("item", SEARCH_SPACE_PARAMETERS)}
       ...
       -- Retention R = exp(-age / S) mirrors memory-retention-score.ts: age from creation or the
       -- later last reinforcement (GREATEST skips NULL), never the event date; S widens with reinforcement.
       CROSS JOIN LATERAL (
         SELECT exp(
    ...
    const blocked = await database().query<{ selected_claim_id: string }>(
      `SELECT DISTINCT selected.id AS selected_claim_id
       ...
       WHERE conflict.family_id = $1 AND conflict.resolution = 'unresolved'
         AND NOT COALESCE(
           partner.claim_status = 'active' AND ${authorizedClaimPredicate("partner", CLOSURE_SPACE_PARAMETERS)},
           false
         )`,
      [auth.familyId, auth.scopes, auth.userId, auth.groupId, selectedIds, ...memorySpaceParameters(auth)],
    );
      ...
      `SELECT conflict.conflict_ref, a.id AS a_id, b.id AS b_id,
       ...
       WHERE conflict.family_id = $1 AND conflict.resolution = 'unresolved'
         ...
         AND (a.id = ANY($5::uuid[]) OR b.id = ANY($5::uuid[]))
          AND ${authorizedClaimPredicate("a", CLOSURE_SPACE_PARAMETERS)}
          AND ${authorizedClaimPredicate("b", CLOSURE_SPACE_PARAMETERS)}
    ...
    const finalAuthorization = await database().query<{
      ...
      `SELECT item.id,
       ...
       WHERE item.family_id = $1 AND item.claim_status = 'active'
         ...
         AND ${authorizedClaimPredicate("item", CLOSURE_SPACE_PARAMETERS)}
         AND NOT EXISTS (
           SELECT 1
      ...
      [auth.familyId, auth.scopes, auth.userId, auth.groupId, outputClaimIds, ...memorySpaceParameters(auth)],
    );
