 ...
 *
 * Key constructs:
 * - External session-budget continuations and questions fail before parking or Telegram delivery.
 */
import {
...
function assertInputRequestPolicy(data: InputRequestedData, ctx: Pick<SessionContext, "session">) {
  ...
  // ask_question is denied in external groups, but Eve authors its prompt before any tool-surface
  // denial runs, so the public chat saw the "preparing a confirmation" placeholder first. A
  // question has no single accountable addressee in a public chat; it is refused here, before
  // presentation, parking, persistence or any Telegram message. Tool approvals keep their path.
  const requestsQuestion = data.requests.some((request) =>
    request.kind === "question" || request.action.toolName === ASK_QUESTION_TOOL_NAME
  );
  if (!requestsQuestion) return;
  throw new AppError(
    "AGENT_EXTERNAL_QUESTION_FORBIDDEN",
    "В общем чате агент не задаёт вопросы через отдельный запрос. Уточните задачу обычным сообщением",
  );
}
