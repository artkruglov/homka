 ...
 *   freshly revalidated auth the interactive callback path uses; an approver who can no longer be
 *   proven yields a claim without auth, which the sweep cancels instead of resuming.
...
import { lockTimeoutAudiences } from "./approval-timeout-audience.js";
...
export const approvalTimeoutRepository: ApprovalTimeoutRepository = {
  async claimExpired(now, timeoutMilliseconds) {
    ...
    try {
      ...
      const audience = await lockTimeoutAudiences(client, now, timeoutMilliseconds, TELEGRAM_HITL_TIMEOUT_SWEEP_BATCH_SIZE);
      const result = await client.query<ClaimRow>(
        `WITH candidate AS (
           SELECT approval.id
            ...
            WHERE approval.id=ANY($5::uuid[])
              AND (session.space_policy_version IS NULL OR session.space_id=ANY($6::uuid[]))
         ...
         RETURNING approval.application_session_id, approval.eve_session_id, approval.id,
                   ...
                   session.space_id, session.space_policy_version`,
        [
          now,
          timeoutMilliseconds,
          TELEGRAM_HITL_TIMEOUT_SWEEP_BATCH_SIZE,
          TELEGRAM_HITL_TIMEOUT_LEASE_MS,
          audience.approvalIds,
          audience.spaceIds,
        ],
      );
      ...
      for (const row of result.rows) {
        ...
        // required role or space cannot be represented, so the claim travels without auth: the
        // sweep cancels it terminally instead of leaving the turn parked forever.
        claims.push(toClaim(row, await resolveCurrentApprovalAuth(client, row)));
      }
