...
it("shows the verified role target and rejects a stale policy before approval",async()=>{
  const memberRef="00000000-0000-4000-8000-000000000002";
  const findSpaceMembers=vi.fn().mockResolvedValue({areaRef:SCHEDULE_ID,title:"Семейные выходные",policyVersion:4,
    members:[{memberRef,name:"Даня",role:"adult"}]});
  const present=createTelegramApprovalPresenter({findSpaceMembers,findSchedule:vi.fn(),findGmailMessage:vi.fn()});
  const request={action:{callId:"role-call",kind:"tool-call" as const,toolName:"manage_space",
    input:{action:"set_role",areaRef:SCHEDULE_ID,memberRef,policyVersion:4,role:"child"}},
    display:"confirmation" as const,options:[],prompt:"Approve",requestId:"role-request"};
  const result=await present(request,context());
  expect(result.prompt).toContain("Даня");
  expect(result.prompt).toContain("Семейные выходные");
  expect(result.prompt).toContain("child");
  expect(result.prompt).not.toContain(memberRef);
  await expect(present({...request,action:{...request.action,input:{...request.action.input,policyVersion:3}}},context()))
