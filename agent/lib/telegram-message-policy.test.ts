 ...
 *
 * Constructs covered:
 * - `isMessageAddressedToBot`: ignores slash commands and accepts configured names at Unicode word boundaries.
...
describe("isMessageAddressedToBot", () => {
  ...
  it.each([
    ...
    "Хомочка, ответь",
    "купили хомяка",
  ])("ignores another agent or an unconfigured name in ordinary group text: %s", (text) => {
  ...
  it.each([
    "премия объявлена",
    "вдохновились химией",
    "Osinaras is a surname",
    "_Осинара",
    `Osinara\u0301s`,
    `Осинара\u200Ds`,
    `Осинара\u203Fs`,
  ])("does not match an agent name inside one Unicode word: %s", (text) => {
    expect(isMessageAddressedToBot({ ...groupMessage, text }, "family_agent")).toBe(false);
  });
