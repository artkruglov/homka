...
/**
 * Семейная группа в режиме `all` слышит реплики без обращения (T03 в task-user-stories): фраза
 * «надо повесить шторы» должна стать делом. Всё остальное без обращения — молчание, иначе бот
 * вмешивался бы в каждую семейную реплику. Во внешних группах неадресованного хода не бывает.
 */
export const FAMILY_UNADDRESSED_RULES = `Поле \`triggeredBy\` текущего сообщения называет причину хода. При \`unaddressed\` тебя не звали, этот чат слышит все реплики: действуй, только если в сообщении есть дело, просьба или обязательство для реестра дел, тогда запиши его и одной фразой скажи, что записано и кто видит; на всё остальное ответь ровно \`<telegram-silent>\`, без комментариев и шуток.`;
