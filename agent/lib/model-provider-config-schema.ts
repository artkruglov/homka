...
const modelProviderConfigSchema = z.object({
  ...
  voice: z.discriminatedUnion("enabled", [
    ...
    z.object({ enabled: z.literal(true), transcriptionModelId: modelIdSchema, provider: z.enum(["groq", "gigaam"]).optional() }).strict(),
  ]),
}).strict().superRefine((config, context) => {
  if (config.voice.enabled && config.voice.provider === "gigaam" && config.voice.transcriptionModelId !== "multilingual_ctc") {
    context.addIssue({ code: "custom", message: "Local speech model mismatch", path: ["voice", "transcriptionModelId"] });
  }
...
export function validateModelProviderRuntimeEnvironment(
  ...
  if (config.voice.enabled && config.voice.provider === "gigaam" && !environment.GIGAAM_API_KEY?.trim()) {
    throw new AppError("AGENT_GIGAAM_API_KEY_REQUIRED", "Для локального распознавания задайте GIGAAM_API_KEY");
  }
  if (config.voice.enabled && config.voice.provider !== "gigaam" && (groqApiKey === undefined || groqApiKey.trim().length === 0)) {
    throw new AppError(
