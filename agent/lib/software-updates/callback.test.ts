...
describe("software update callback handler", () => {
  it("commits an exact owner approval and then removes the keyboard", async () => {
    ...
    const answerInitiative = vi.fn().mockResolvedValue(undefined);
    const handle = createSoftwareUpdateCallbackHandler({ answerInitiative, repository, transport });

    await expect(handle(callbackQuery())).resolves.toBe(true);

    // Нажатая кнопка это ответ человека: пауза по молчанию не должна расти от отвеченного.
    expect(answerInitiative).toHaveBeenCalledWith("101");
  ...
  it("rejects a foreign owner callback without exposing it to Eve", async () => {
    ...
    const handle = createSoftwareUpdateCallbackHandler({
      answerInitiative: vi.fn().mockResolvedValue(undefined), repository, transport,
    });
