...
interface SoftwareUpdateCallbackDependencies {
  ...
  /** Нажатая кнопка это ответ человека: она снимает паузу по молчанию так же, как сообщение. */
  answerInitiative(telegramUserId: string): Promise<void>;
}
...
export function createSoftwareUpdateCallbackHandler(
  ...
  return async function handleSoftwareUpdateCallback(
    ...
    // Человек ответил, пусть и кнопкой: счёт неотвеченных предложений обнуляется. Сбой здесь
    // не отменяет уже принятого решения, поэтому он только логируется.
    try {
      await dependencies.answerInitiative(query.from.id);
    } catch (error) {
      console.error(JSON.stringify({
        code: "AGENT_INITIATIVE_ANSWER_FAILED",
        error: error instanceof Error ? error.message : String(error),
      }));
    }
...
export const handleSoftwareUpdateCallback = createSoftwareUpdateCallbackHandler({
  answerInitiative: (telegramUserId) =>
    initiativeRepository.markAnsweredByTelegramId(telegramUserId, new Date()),
