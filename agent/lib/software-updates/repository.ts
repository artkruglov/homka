...
export function createSoftwareUpdateRepository(
  ...
  async findCurrentOwner() {
    const result = await database().query<{
      ...
      `SELECT fm.family_id, fm.user_id, u.telegram_user_id, settings.timezone,
              to_char(settings.quiet_start, 'HH24:MI') AS quiet_start,
              to_char(settings.quiet_end, 'HH24:MI') AS quiet_end
         FROM family_memberships fm
         JOIN users u ON u.id = fm.user_id
         LEFT JOIN user_notification_settings settings ON settings.user_id = u.id
        WHERE fm.role = 'owner'
    ...
    return {
      ...
      quietEnd: owner.quiet_end,
      quietStart: owner.quiet_start,
