...
describe("buildSandboxContainerOptions", () => {
  it.each(["workspaces", "tools"] as const)("rejects a linked %s root before inspecting or starting compute", async (kind) => {
    const root = await mkdtemp(join(tmpdir(), "osinara-sandbox-root-"));
    temporaryRoots.push(root);
    await mkdir(join(root, kind), { recursive: true });
    await mkdir(join(root, "other-audience"));
    await symlink(join(root, "other-audience"), join(root, kind, PERSONAL_WORKSPACE_ID));
    const getContainer = vi.fn(() => ({ inspect: async () => { throw { statusCode: 404 }; } }));
    const createContainer = vi.fn();
    const engine = createDockerSandboxEngine({
      docker: { getContainer, createContainer } as unknown as Docker,
      roots: { toolsRoot: join(root, "tools"), workspaceRoot: join(root, "workspaces") },
      runtime,
    });
    await expect(engine.createSession({
      access: "trusted", eveSessionId: EVE_SESSION_ID,
      mounts: [{ mountPoint: "personal", workspaceId: PERSONAL_WORKSPACE_ID }],
      sandboxSessionId: SANDBOX_SESSION_ID, seedDigest: EMPTY_SEED_DIGEST,
    })).rejects.toThrow(kind === "tools" ? "AGENT_SANDBOX_RUNNER_TOOLS_MISSING" : "AGENT_SANDBOX_RUNNER_WORKSPACE_MISSING");
