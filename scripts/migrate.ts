 ...
 * - Refuses to start an image whose migrations are older than the schema it found.
 */
...
import { requireSchemaWithinImage } from "./migration-data/schema-image-guard.ts";
...
try {
  ...
  for (const name of migrations) {
    ...
    try {
      ...
      if (name === "104_space_record_bindings.sql") await backfillSpaceRecords104(client);
  ...
  await requireSchemaWithinImage(client, migrations);
} finally {
  await client.end();
}
