...
export async function patchWorkflowTransport(replace: (path: string, before: string, after: string) => Promise<void>) {
  ...
  const runtime = stripTypeScriptTypes(await readFile("scripts/eve-runtime/workflow-transport.ts", "utf8"));
  ...
  // Workflow executes steps inline in a flow HTTP request. With a 35-minute transport window, the
  // package's whole-run serialization would hold every later replay of the run (a cancellation or a
  // hook resume) behind that request for the whole turn. Storage keeps atomic step and event-slot
  // claims, and the fence above still joins a redelivery of the same live message (upstream 795ae11).
