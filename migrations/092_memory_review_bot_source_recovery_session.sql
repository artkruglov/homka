-- Продолжение 091. Повторный запуск батча 72–87 (группа ботов) упал на подготовке сессии:
-- `prepare` вставляет сессию проверки с ON CONFLICT по memory_review_batch_id, а прежняя сессия
-- этого батча уже закрыта и привязана к ходу Eve, поэтому вставка не возвращает строку.
-- Это штатная защита от повторного прогона после запуска модели. Здесь повтор безопасен: ход
-- упал на turn.started до привязки источников, записей в память не было (проверено ниже).
-- Отвязываем закрытую сессию от батча и возвращаем батч в очередь ещё раз.
DO $$
DECLARE
  ...
  recovery_diagnostic_code constant text := 'AGENT_MEMORY_REVIEW_BOT_SOURCE_SESSION_RECOVERY';
...
BEGIN
  ...
  IF incident.lane_id IS DISTINCT FROM incident_lane_id
    ...
    RAISE EXCEPTION
      'AGENT_MEMORY_REVIEW_BOT_SOURCE_SESSION_RECOVERY_STATE_INVALID: Incident batch changed since inspection';
  END IF;
  ...
  IF lane_cursor IS DISTINCT FROM 71 THEN
    RAISE EXCEPTION
      'AGENT_MEMORY_REVIEW_BOT_SOURCE_SESSION_RECOVERY_LANE_INVALID: Review lane moved past the incident';
  END IF;
      ...
      SELECT 1 FROM memory_mutation_operations AS operation
       WHERE operation.eve_session_id IN (
         SELECT eve_session_id FROM conversation_sessions
          WHERE memory_review_batch_id = incident_batch_id AND eve_session_id IS NOT NULL
       )
    ) THEN
    RAISE EXCEPTION
      'AGENT_MEMORY_REVIEW_BOT_SOURCE_SESSION_RECOVERY_SIDE_EFFECT: Incident batch already wrote memory';
  END IF;

  -- The closed session keeps its history; only the batch link is released for a fresh session.
  UPDATE conversation_sessions
     SET memory_review_batch_id = NULL
   WHERE memory_review_batch_id = incident_batch_id AND retired_at IS NOT NULL;
